...
@section('content')
  <div id="campaign_add_users" class="row">
    <div class="small-12 columns">
      <p>Add registered users to this campaign. <br><br>FUTURE: Users who are already registered will be added, and any
        users who are not registered will receive an email to register.</p>

      <div id="campaign_users_add" class="small-12 columns">
        <form id="campaign_add_users_form" method="POST" action="{{ url('campaigns/' . $campaign->id . '/add_users') }}"
              class="row" data-abide>
            ...
            <div class="row">
              <div class="small-10 columns">
                <input type="text" name="email"
                       ...
                       class="small-8 columns" required/>
            ...
            <tr>
              ...
              <th>
                                    <span data-tooltip aria-haspopup="true" class="has-tip tip-right"
                                          title="{{ Lang::get('campaign.role') }}">Role <i
                ...
                <td>
                  @if ( $campaign_user->id != $user->id )
                    ...
                    <select name="role[{{ $campaign_user->id }}]">
                      <option value="{{ config('roles.admin') }}" {{ ($title == 'Admin') ? 'selected' : '' }}>Admin
                      </option>
                      <option value="{{ config('roles.user') }}" {{ ($title == 'User') ? 'selected' : '' }}>User
                      </option>
                    </select>
                  @else
