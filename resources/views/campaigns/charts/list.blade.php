...
@foreach ( $campaigns as $campaign )
    ...
    <div class = "small-12 columns">
        <div class = "panel">
            ...
            @else
                <div id = "campaigns_list_chart_{{ $campaign->id }}" class = "campaigns_list_chart_item panel animated fadeIn text-center">
            ...
            <div class = "campaigns_list_owner row">
                    ...
                    <div id = "campaigns_list_users" class = "inline">
                        ...
                        <div id = "campaigns_list_users_icons" class = "inline">
                            ...
                            @foreach ( $campaign->users as $campaign_user )
                                <span data-tooltip aria-haspopup = "true" class = "has-tip tip-right" title = "{{ $campaign_user->fullname }}">
                                    @if ( is_null($campaign_user->profile_url) )
                                    <img src = "{{ asset('img/default.png') }}" class = "th" />
                                    @else
                                    <img src = "{{ asset('img/user/' . $campaign_user->id . '/' . $campaign_user->profile_url) }}" class = "th" />
                                    @endif
                                </span>
                            @endforeach
                ...
                <div class = "icons small-2 columns text-right">
                    <span data-tooltip aria-haspopup = "true" class = "has-tip tip-left" title = "{{ Lang::get('campaign.comments') }}"><i class = "fa fa-comments"></i> {{ number_format($campaign->comments->count()) }}</span>
                    <span data-tooltip aria-haspopup = "true" class = "has-tip tip-left" title = "{{ Lang::get('campaign.total_lead_count') }}"><i class = "fa fa-file-text-o"></i> {{ number_format($campaign->leads->count()) }}</span>
                </div>
            </div>
        </div>
    </div>
