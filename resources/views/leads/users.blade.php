        ...
        <div class="inline avatar">
          <a class="th" href="{{ url('users/' . $owner->id) }}">
            @if ( is_null($owner->profile_url) )
              <img src="{{ asset('img/default.png') }}" class="owner" data-tooltip aria-haspopup="true"
                   class="has-tip tip-top" title="{{ Lang::get('lead.owner') }}"/>
            @else
              ...
              <div class="inline avatar">
                <a class="th" href="{{ url('users/' . $watcher->id) }}">
                  @if ( is_null($watcher->profile_url) )
                    <img src="{{ asset('img/default.png') }}" class="watcher" data-tooltip aria-haspopup="true"
                         class="has-tip tip-top" title="{{ $watcher->fullname }}"/>
                  @else
...
@else
  ...
  @if ( $has_admin_access )
    <div class="name inline">
      <h5>
        <strong><i class="fa fa-exclamation red"></i>&nbsp;&nbsp;This lead is currently not assigned to anyone. Click <a
            href="{{ url('leads/' . $lead->id . '/assign_lead') }}" class="underline">here</a> to assign an
          owner.</strong>
      </h5>
    </div>
  @endif
