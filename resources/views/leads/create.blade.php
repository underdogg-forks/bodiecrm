  ...
  <div id="leads_single_create" class="row">
          ...
          <div class="small-12 columns">
            <label for="landing_page">
                            <span data-tooltip aria-haspopup="true" class="has-tip tip-right"
                                  title="{{ Lang::get('lead.landing_page') }}">Landing Page <i
                                class="fa fa-question-circle"></i>
                                <span class="label right">Required</span>
            </label>
            <select id="landing_page_select" name="landing_page_id" data-url="{{ url('landing_pages/get_auth_key') }}"
                    data-csrf="{!! csrf_token() !!}" required>
              <option value="">Select One</option>
              @foreach ( $user->landing_pages_active() as $landing_page )
                <option value="{{ $landing_page->id }}"
                        data-key="{{ $landing_page->auth_key }}">{{ $landing_page->title }}</option>
              @endforeach
            </select>
    ...
    @else
      <div id="leads_single_create_empty" class="text-center">
        <h3>You have no active landing pages to create a lead!<br>Create one in a <a href="{{ url('campaigns') }}">Campaign</a>
          or ask an admin to add you.</h3>
      </div>
    @endif
  </div>
