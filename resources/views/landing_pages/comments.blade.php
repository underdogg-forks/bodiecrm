<form id="comment_submit" method="POST" action="{{ url('landing_pages/' . $landing_page->id . '/add_comment') }}"
    ...
    @foreach ( $landing_page->comments as $comment )
      <div class="comment_item">
                <span data-tooltip aria-haspopup="true" class="has-tip tip-right"
                  ...
                  @else
                    <img src="{{ asset('img/user/' . $comment->user->id . '/' . $comment->user->profile_url) }}"
                         class="th"/>
                  @endif
                </span>
