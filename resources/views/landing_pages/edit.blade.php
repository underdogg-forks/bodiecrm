        ...
        <div class = "small-12 columns">
            ...
            <form method = "POST" action = "{{ url('landing_pages/' . $landing_page->id) }}" class = "row" data-abide>
                ...
                <div id = "send_email_radio" class = "small-12 columns">
                    <label for = "send_email">Send notification email when a lead is submitted? <span data-tooltip aria-haspopup = "true" class = "has-tip tip-right" title = "{{ Lang::get('landing_page.send_notification_email') }}"><i class = "fa fa-question-circle"></i></span></label>
                    <input type = "radio" name = "send_email" value = 1 id = "send_email_yes" data-target = "send_email_yes_details" data-action = "unhide" {{ $landing_page->send_email ? 'checked' : '' }} /><label for = "send_email_yes">Yes</label>
                    <input type = "radio" name = "send_email" value = 0 id = "send_email_no" data-target = "send_email_yes_details" data-action = "hide" {{ ! $landing_page->send_email ? 'checked' : '' }} /><label for = "send_email_no">No</label>
                    <br><br>
                </div>
                ...
                <div id = "send_email_yes_details" class = "small-12 columns {{ ! $landing_page->send_email ? 'hide' : '' }}">
                    <div>
                        ...
                        <input type = "text" name = "email_title" placeholder = "Default email title: Lead submitted for [Landing Page Title]" value = "{{ $landing_page->email_title }}" />
                    </div>
                    <div>
                        <label for = "email_to">Send notification mail to <span data-tooltip aria-haspopup = "true" class = "has-tip tip-right" title = "{{ Lang::get('landing_page.send_notification_email_users') }}"><i class = "fa fa-question-circle"></i></span></label>
                        <select class = "chosen-select" name = "add_user_email_notification[]" multiple>
                            @foreach ( $landing_page->campaign->users as $campaign_user )
                                <option value = "{{ $campaign_user->id }}" {{ $users_to_email->contains($campaign_user->id) ? 'selected' : '' }}>
