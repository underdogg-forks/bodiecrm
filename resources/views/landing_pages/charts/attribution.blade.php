...
@foreach ( $landing_page->attribution as $key => $value )
    <div id = "landing_page_single_{{ $key }}" class = "small-3 columns text-center">
        <h5>
            <span data-tooltip aria-haspopup = "true" class = "has-tip tip-top" title = "{{ Lang::get('landing_page.' . $key) }}">{{ ucwords(str_replace('_', ' ', $key)) }} <i class = "fa fa-question-circle"></i></span>
        </h5>
        ...
        @else
            <div id = "landing_page_single_{{ $key }}_chart-{{ $landing_page->id}}" class = "landing_page_single_donut_chart chart_no_data donut animated fadeIn">
                <h4>No data</h4>
            </div>
        @endif
    </div>
